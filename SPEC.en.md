...
- **@mention notifications (お知らせ)**: typing `@` in the comment body (or the `@` button) shows project members; picking one inserts `@Name` into the text and adds the user to the notify set. The notify set is shown as removable **chips** below the box and is the single source of truth (editing the text never changes it). On post, `notifiedUserId[]` is sent so Backlog notifies those users. The `@Name` in the body is plain readable text. **Mentions created in Backlog's own web UI render as `<a data-linktype="mention" ...>` links to the user's page; mentions created via the REST API (including this app) do not** (verified against a real account: creating the identical mention text via the web UI and via this app/the REST API and comparing the resulting HTML — the web-UI version was wrapped in `<a data-linktype="mention" href="/user/<userId>" ...>`, the API version stayed plain text). Notification delivery via `notifiedUserId[]` works correctly either way. The linkification likely depends on extra, non-public data the web UI's mention picker sends alongside the form post — something the public REST API has no parameter for, so it cannot be reproduced from outside Backlog's own client.
...
- **`@mentions` in REST API comments don't link to the user's page**: a mention created in Backlog's own web UI renders as `<a data-linktype="mention" href="/user/<userId>" ...>` around the `@Name` text, but a comment created via the REST API (`POST /issues/:key/comments`, including this app) — with identical text and the correct `notifiedUserId[]` — renders as plain text instead (verified against a real account by creating the same mention both ways and comparing the resulting HTML). Notification delivery via `notifiedUserId[]` works correctly either way. Linkification likely depends on extra, non-public data the web UI's mention picker sends alongside the form post, which the public REST API has no parameter for.
